...
export function IssueProperties({
  ...
  const reviewerTrigger = reviewerValues.length > 0
    ...
    : <span className="text-sm text-muted-foreground">{t("issueProperties.none", { defaultValue: "None" })}</span>;
  ...
  const labelsTrigger = selectedIssueLabels.length > 0 ? (
    ...
    <>
      ...
      <span className="text-sm text-muted-foreground">{t("issueProperties.noLabels", { defaultValue: "No labels" })}</span>
    </>
  );
  const labelsExtra = (issue.labelIds ?? []).length > 0 ? (
    <button
      ...
      aria-label={t("issueProperties.addLabel", { defaultValue: "Add label" })}
      title={t("issueProperties.addLabel", { defaultValue: "Add label" })}
    >
  ...
  const labelsContent = (
    <>
      <input
        ...
        placeholder={t("issueProperties.searchLabels", { defaultValue: "Search labels..." })}
        value={labelSearch}
      ...
      <div className="mt-2 border-t border-border pt-2 space-y-1">
        <div className="flex items-center gap-1">
          ...
          <input
            ...
            placeholder={t("issueProperties.newLabel", { defaultValue: "New label" })}
            value={newLabelName}
        ...
        >
          ...
          {createLabel.isPending
            ? t("issueProperties.creatingLabel", { defaultValue: "Creating..." })
  ...
  const assigneeTrigger = assignee ? (
    ...
    <>
      <User className="h-3.5 w-3.5 text-muted-foreground" />
      <span className="text-sm text-muted-foreground">{t("issueProperties.unassigned", { defaultValue: "Unassigned" })}</span>
    </>
  );
    ...
    [
      {
        id: "",
        kind: "none" as const,
        label: t("issueProperties.noAssignee", { defaultValue: "No assignee" }),
        searchText: "",
      },
      ...(currentUserId
        ? [{
      ...
      ...(issue.createdByUserId && issue.createdByUserId !== currentUserId
        ? [{
            ...
            label: creatorUserLabel
              ? t("issueProperties.assignToUser", { defaultValue: "Assign to {{user}}", user: creatorUserLabel })
  ...
  const assigneeContent = (
    <>
      <input
        className="w-full px-2 py-1.5 text-xs bg-transparent outline-none border-b border-border mb-1 placeholder:text-muted-foreground/50"
        placeholder={t("issueProperties.searchAssignees", { defaultValue: "Search assignees..." })}
  ...
  const projectTrigger = issue.projectId ? (
    ...
    <>
      ...
      <span className="text-sm text-muted-foreground">{t("issueProperties.noProject", { defaultValue: "No project" })}</span>
    </>
  );
    ...
    [
      {
        id: "",
        kind: "none" as const,
        name: t("issueProperties.noProject", { defaultValue: "No project" }),
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1">
        <PropertyRow label={t("issueProperties.status", { defaultValue: "Status" })}>
          <StatusIcon
            status={issue.status}
        ...
        <PropertyRow label={t("issueProperties.priority", { defaultValue: "Priority" })}>
          <PriorityIcon
        ...
        <PropertyPicker
          inline={inline}
          label={t("issueProperties.assignee", { defaultValue: "Assignee" })}
          open={assigneeOpen}
        ...
        <PropertyPicker
          inline={inline}
          label={t("issueProperties.project", { defaultValue: "Project" })}
          open={projectOpen}
        ...
        {inline ? (
          <div>
            <PropertyRow label={t("issueProperties.blockedBy", { defaultValue: "Blocked by" })}>
        ...
        <PropertyRow label={t("issueProperties.blocking", { defaultValue: "Blocking" })}>
        ...
        <PropertyRow label={t("issueProperties.subIssues", { defaultValue: "Sub-issues" })}>
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {onAddSubIssue ? (
              ...
              >
                <Plus className="h-3 w-3" />
              {t("issueProperties.addSubIssue", { defaultValue: "Add sub-issue" })}
              </button>
            ) : null}
          </div>
        </PropertyRow>
        ...
        {relatedTasks.length > 0 ? (
          <PropertyRow label={t("issueProperties.relatedTasks", { defaultValue: "Related Tasks" })}>
            <div className="flex flex-wrap gap-1">
      ...
      {liveWorkspaceService || issue.currentExecutionWorkspace?.branchName || issue.currentExecutionWorkspace?.cwd || issue.executionWorkspaceId ? (
        <>
          ...
          <div className="space-y-1">
            ...
            {workspaceFilterId && (
              <PropertyRow label={t("issueProperties.tasks", { defaultValue: "Tasks" })}>
                <Link
                  to={issuesWorkspaceFilterHref(workspaceFilterId)}
                  className="text-sm text-primary hover:underline inline-flex items-center gap-1"
                >
                  {t("issueProperties.viewWorkspaceTasks", { defaultValue: "View workspace tasks" })}
                  <ExternalLink className="h-3 w-3" />
                </Link>
              </PropertyRow>
            )}
