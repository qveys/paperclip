...
export function ExecutionWorkspaceCloseDialog({
  ...
  const closeWorkspace = useMutation({
    ...
    onSuccess: (workspace) => {
      ...
      pushToast({
        title: currentStatus === "cleanup_failed" ? t("executionWorkspaceCloseDialog.closeRetried") : t("executionWorkspaceCloseDialog.closedSuccess"),
        tone: "success",
      });
      onOpenChange(false);
      onClosed?.(workspace);
    ...
    onError: (error) => {
      pushToast({
        title: t("executionWorkspaceCloseDialog.failedToClose"),
      ...
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription className="break-words">
            <Trans
              i18nKey="executionWorkspaceCloseDialog.description"
              ns="core"
        ...
        {readinessQuery.isLoading ? (
          <div className="flex items-center gap-2 rounded-xl border border-border bg-background px-4 py-3 text-sm text-muted-foreground">
            ...
            {tx("executionWorkspaceCloseDialog.checkingReadiness")}
          </div>
        ) : readinessQuery.error ? (
          <div className="rounded-xl border border-destructive/30 bg-destructive/5 px-4 py-3 text-sm text-destructive">
            {readinessQuery.error instanceof Error ? readinessQuery.error.message : t("executionWorkspaceCloseDialog.failedToInspect")}
          </div>
        ) : readiness ? (
          <div className="space-y-4">
            <div className={`rounded-xl border px-4 py-3 text-sm ${readinessTone(readiness.state)}`}>
              ...
              <div className="mt-1 text-xs opacity-80">
                {readiness.isSharedWorkspace
                  ? t("executionWorkspaceCloseDialog.sharedWorkspaceMessage")
                  : readiness.git?.workspacePath && readiness.git.repoRoot && readiness.git.workspacePath !== readiness.git.repoRoot
                    ? t("executionWorkspaceCloseDialog.ownCheckoutMessage")
                    : readiness.isProjectPrimaryWorkspace
                      ? t("executionWorkspaceCloseDialog.primaryWorkspaceMessage")
                      : t("executionWorkspaceCloseDialog.disposableMessage")}
              </div>
            </div>

            {blockingIssues.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{tx("executionWorkspaceCloseDialog.blockingIssuesTitle")}</h3>
            ...
            {readiness.blockingReasons.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{tx("executionWorkspaceCloseDialog.blockingReasonsTitle")}</h3>
            ...
            {readiness.warnings.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{tx("executionWorkspaceCloseDialog.warnings")}</h3>
                <ul className="space-y-2 text-sm text-muted-foreground">
            ...
            {readiness.git ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{tx("executionWorkspaceCloseDialog.gitStatus")}</h3>
                <div className="rounded-xl border border-border bg-background px-4 py-3 text-sm">
                  <div className="grid gap-2 sm:grid-cols-2">
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{tx("executionWorkspaceCloseDialog.branch")}</div>
                      <div className="font-mono text-xs">{readiness.git.branchName ?? t("executionWorkspaceCloseDialog.unknown")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{tx("executionWorkspaceCloseDialog.baseRef")}</div>
                      <div className="font-mono text-xs">{readiness.git.baseRef ?? t("executionWorkspaceCloseDialog.notSet")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{tx("executionWorkspaceCloseDialog.mergedIntoBase")}</div>
                    ...
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{tx("executionWorkspaceCloseDialog.aheadBehind")}</div>
                      <div>
                        {(readiness.git.aheadCount ?? 0).toString()} / {(readiness.git.behindCount ?? 0).toString()}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{tx("executionWorkspaceCloseDialog.dirtyTrackedFiles")}</div>
                      <div>{readiness.git.dirtyEntryCount}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{tx("executionWorkspaceCloseDialog.untrackedFiles")}</div>
            ...
            {otherLinkedIssues.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{tx("executionWorkspaceCloseDialog.otherLinkedIssues")}</h3>
                <div className="space-y-2">
            ...
            {readiness.runtimeServices.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{tx("executionWorkspaceCloseDialog.attachedRuntimeServices")}</h3>
                <div className="space-y-2">
                  {readiness.runtimeServices.map((service) => (
                    <div key={service.id} className="rounded-xl border border-border bg-background px-4 py-3 text-sm">
                      ...
                      <div className="mt-1 break-words text-xs text-muted-foreground">
                        {service.url ?? service.command ?? service.cwd ?? t("executionWorkspaceCloseDialog.noAdditionalDetails")}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <section className="space-y-2">
              <h3 className="text-sm font-medium">{tx("executionWorkspaceCloseDialog.cleanupActions")}</h3>
              <div className="space-y-2">
            ...
            {currentStatus === "cleanup_failed" ? (
              <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 px-4 py-3 text-sm text-muted-foreground">
                {tx("executionWorkspaceCloseDialog.cleanupRetryNotice")}
              </div>
            ) : null}

            {currentStatus === "archived" ? (
              <div className="rounded-xl border border-border bg-background px-4 py-3 text-sm text-muted-foreground">
                {tx("executionWorkspaceCloseDialog.alreadyArchived")}
              </div>
            ) : null}

            {readiness.git?.repoRoot ? (
              <div className="break-words text-xs text-muted-foreground">
                {tx("executionWorkspaceCloseDialog.repoRoot")}: <span className="font-mono break-all">{readiness.git.repoRoot}</span>
                {readiness.git.workspacePath ? (
                  <>
                    {" · "}{tx("executionWorkspaceCloseDialog.workspacePath")}: <span className="font-mono break-all">{readiness.git.workspacePath}</span>
                  </>
                ) : null}
              </div>
            ) : null}
        ...
        <DialogFooter>
          ...
          >
            {tx("executionWorkspaceCloseDialog.cancel")}
          </Button>
          <Button
