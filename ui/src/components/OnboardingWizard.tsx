...
export function OnboardingWizard() {
  ...
  async function runAdapterEnvironmentTest(
    ...
    if (!createdCompanyId) {
      setAdapterEnvError(
        t("onboarding.errors.createOrSelectCompanyBeforeEnvTest", {
          defaultValue: "Create or select a company before testing adapter environment."
        })
      );
      return null;
    }
    ...
    try {
      ...
      setAdapterEnvError(
        err instanceof Error
          ? err.message
          : t("onboarding.errors.adapterEnvironmentTestFailed", {
              defaultValue: "Adapter environment test failed"
            })
      );
      return null;
    } finally {
  ...
  async function handleStep1Next() {
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : t("onboarding.errors.failedToCreateCompany", {
              defaultValue: "Failed to create company"
            })
      );
    } finally {
      setLoading(false);
  ...
  async function handleStep2Next() {
    ...
    try {
      if (adapterType === "opencode_local") {
        ...
        if (!selectedModelId) {
          setError(
            t("onboarding.errors.opencodeModelRequired", {
        ...
        if (adapterModelsError) {
          setError(
            adapterModelsError instanceof Error
              ...
              : t("onboarding.errors.failedToLoadOpenCodeModels", {
        ...
        if (adapterModelsLoading || adapterModelsFetching) {
          setError(
            t("onboarding.errors.opencodeModelsStillLoading", {
        ...
        if (!discoveredModels.some((entry) => entry.id === selectedModelId)) {
          setError(
            discoveredModels.length === 0
              ? t("onboarding.errors.noOpenCodeModelsDiscovered", {
                  defaultValue:
                    "No OpenCode models discovered. Run `opencode models` and authenticate providers."
                })
              : t("onboarding.errors.opencodeConfiguredModelUnavailable", {
                  defaultValue: "Configured OpenCode model is unavailable: {{modelId}}",
      ...
      setError(
        err instanceof Error
          ? err.message
          : t("onboarding.errors.failedToCreateAgent", {
              defaultValue: "Failed to create agent"
            })
      );
    } finally {
      setLoading(false);
  ...
  async function handleUnsetAnthropicApiKey() {
    ...
    try {
      ...
      if (result?.status === "fail") {
        setError(
          t("onboarding.errors.unsetAnthropicRetryStillFailing", {
            defaultValue:
      ...
      setError(
        err instanceof Error
          ? err.message
          : t("onboarding.errors.failedToUnsetAnthropicAndRetry", {
              defaultValue: "Failed to unset ANTHROPIC_API_KEY and retry."
            })
      );
    } finally {
      setUnsetAnthropicLoading(false);
  ...
  async function handleLaunch() {
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : t("onboarding.errors.failedToCreateTask", {
          ...
          >
            <div className="w-full max-w-md mx-auto my-auto px-8 py-12 shrink-0">
              ...
              <div className="flex items-center gap-0 mb-8 border-b border-border">
                {(
                  [
                    { step: 1 as Step, label: t("onboarding.tabs.company", { defaultValue: "Company" }), icon: Building2 },
                    { step: 2 as Step, label: t("onboarding.tabs.agent", { defaultValue: "Agent" }), icon: Bot },
                    { step: 3 as Step, label: t("onboarding.tabs.task", { defaultValue: "Task" }), icon: ListTodo },
                    { step: 4 as Step, label: t("onboarding.tabs.launch", { defaultValue: "Launch" }), icon: Rocket }
                  ] as const
              ...
              {step === 1 && (
                <div className="space-y-5">
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    <div>
                      <h3 className="font-medium">{t("onboarding.nameYourCompany", { defaultValue: "Name your company" })}</h3>
                      <p className="text-xs text-muted-foreground">
                        {t("onboarding.companyHelp", { defaultValue: "This is the organization your agents will work for." })}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="group">
                    ...
                    >
                      {t("onboarding.missionOptional", { defaultValue: "Mission / goal (optional)" })}
                    </label>
                    <textarea
                      className="w-full rounded-md border border-border bg-transparent px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-ring placeholder:text-muted-foreground/50 resize-none min-h-[60px]"
                      placeholder={t("onboarding.companyGoalPlaceholder", { defaultValue: "What is this company trying to achieve?" })}
                      value={companyGoal}
              ...
              {step === 2 && (
                <div className="space-y-5">
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    <div>
                      <h3 className="font-medium">{t("onboarding.createFirstAgent", { defaultValue: "Create your first agent" })}</h3>
                      <p className="text-xs text-muted-foreground">
                        {t("onboarding.agentRunTasksHelp", { defaultValue: "Choose how this agent will run tasks." })}
                      </p>
                    </div>
                  </div>
                  <div>
                    <label className="text-xs text-muted-foreground mb-1 block">
                      {t("onboarding.agentName", { defaultValue: "Agent name" })}
                    </label>
                    <input
                      className="w-full rounded-md border border-border bg-transparent px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-ring placeholder:text-muted-foreground/50"
                      placeholder={t("onboarding.agentNamePlaceholder", { defaultValue: "CEO" })}
                      value={agentName}
                  ...
                  <div>
                    ...
                    >
                      ...
                      {t("onboarding.moreAgentAdapterTypes", {
                        defaultValue: "More Agent Adapter Types"
                      })}
                    </button>
                      ...
                      <div className="flex items-center justify-between gap-2">
                        <div>
                          ...
                          <p className="text-[11px] text-muted-foreground">
                            {t(
                              "onboarding.adapterEnvironmentCheckHelp",
                              {
                                defaultValue:
                      ...
                      {shouldSuggestUnsetAnthropicApiKey && (
                        <div className="rounded-md border border-amber-300/60 bg-amber-50/40 px-2.5 py-2 space-y-2">
                          <p className="text-[11px] text-amber-900/90 leading-relaxed">
                            {t("onboarding.claudeFailedWithAnthropicPrefix", {
                              defaultValue: "Claude failed while"
                            })}{" "}
                            <span className="font-mono">ANTHROPIC_API_KEY</span>{" "}
                            {t("onboarding.claudeFailedWithAnthropicSuffix", {
                              defaultValue:
                ...
                <div className="space-y-5">
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {t("onboarding.readyToLaunchHelp", {
                          defaultValue:
...
function AdapterEnvironmentResult({
  ...
  const statusLabel =
    result.status === "pass"
      ? t("onboarding.adapterEnvironmentStatus.passed", {
          defaultValue: "Passed"
        })
      : result.status === "warn"
      ? t("onboarding.adapterEnvironmentStatus.warnings", {
          defaultValue: "Warnings"
        })
      : t("onboarding.adapterEnvironmentStatus.failed", {
          defaultValue: "Failed"
        });
  const statusClass =
    result.status === "pass"
          ...
          >
            ...
            {check.hint && (
              <span className="block opacity-90 break-words">
                {t("onboarding.hintLabel", { defaultValue: "Hint:" })}{" "}
                {check.hint}
              </span>
            )}
          </div>
