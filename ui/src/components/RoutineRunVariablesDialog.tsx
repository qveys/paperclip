      ...
      <DialogContent className="max-w-xl">
        ...
        <div className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-1.5">
              <Label className="text-xs">{t("routineRunVariablesDialog.agentRequired", { defaultValue: "Agent *" })}</Label>
              <InlineEntitySelector
                value={selection.assigneeAgentId}
                options={assigneeOptions}
                recentOptionIds={recentAssigneeIds}
                placeholder={t("routineRunVariablesDialog.agent", { defaultValue: "Agent" })}
                noneLabel={t("routineRunVariablesDialog.selectAgent", { defaultValue: "Select an agent" })}
                searchPlaceholder={t("routineRunVariablesDialog.searchAgents", { defaultValue: "Search agents..." })}
                emptyMessage={t("routineRunVariablesDialog.noAgentsFound", { defaultValue: "No agents found." })}
                disablePortal
                openOnFocus={false}
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("routineRunVariablesDialog.selectAgent", { defaultValue: "Select an agent" })}</span>
                  )
                }
                renderOption={(option) => {
            ...
            <div className="space-y-1.5">
              <Label className="text-xs">{t("routineRunVariablesDialog.project", { defaultValue: "Project" })}</Label>
              <InlineEntitySelector
                value={selection.projectId}
                options={projectOptions}
                recentOptionIds={recentProjectIds}
                placeholder={t("routineRunVariablesDialog.project", { defaultValue: "Project" })}
                noneLabel={t("routineRunVariablesDialog.noProject", { defaultValue: "No project" })}
                searchPlaceholder={t("routineRunVariablesDialog.searchProjects", { defaultValue: "Search projects..." })}
                emptyMessage={t("routineRunVariablesDialog.noProjectsFound", { defaultValue: "No projects found." })}
                disablePortal
                openOnFocus={false}
                onChange={(projectId) => {
                ...
                renderTriggerValue={(option) =>
                  option && selectedProject ? (
                    ...
                    <span className="text-muted-foreground">{t("routineRunVariablesDialog.noProject", { defaultValue: "No project" })}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
            ...
            <div key={variable.name} className="space-y-1.5">
              ...
              {isAutoWorkspaceBranchVariable(variable) ? (
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="__unset__">{t("routineRunVariablesDialog.noValue", { defaultValue: "No value" })}</SelectItem>
                    <SelectItem value="true">{t("routineRunVariablesDialog.true", { defaultValue: "True" })}</SelectItem>
                    <SelectItem value="false">{t("routineRunVariablesDialog.false", { defaultValue: "False" })}</SelectItem>
                  </SelectContent>
                </Select>
                ...
                >
                  <SelectTrigger>
                    <SelectValue placeholder={t("routineRunVariablesDialog.chooseValue", { defaultValue: "Choose a value" })} />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="__unset__">{t("routineRunVariablesDialog.noValue", { defaultValue: "No value" })}</SelectItem>
        ...
        <DialogFooter showCloseButton={false}>
          {!selection.assigneeAgentId ? (
            <p className="mr-auto text-xs text-amber-600">{t("routineRunVariablesDialog.defaultAgentRequired", { defaultValue: "Default agent required for this run." })}</p>
          ) : missingRequired.length > 0 ? (
            <p className="mr-auto text-xs text-amber-600">
              {t("routineRunVariablesDialog.missing", { defaultValue: "Missing" })}: {missingRequired.join(", ")}
            </p>
          ) : workspaceSelectionEnabled && !workspaceConfigValid ? (
            <p className="mr-auto text-xs text-amber-600">
              {t("routineRunVariablesDialog.chooseWorkspace", { defaultValue: "Choose an existing workspace before running." })}
            </p>
          ) : (
