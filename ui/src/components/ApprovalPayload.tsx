...
export function approvalLabel(
  ...
  const hasT = typeof tOrType === "function";
  const t = hasT ? (tOrType as TFunction) : null;
  const type = hasT ? (typeOrPayload as string) : (tOrType as string);
  const payload = hasT
    ? maybePayload ?? null
    : ((typeOrPayload as Record<string, unknown> | null | undefined) ?? null);
  const key = typeLabelKey[type];
  const base = key && t ? (t(key) as string) : type;
