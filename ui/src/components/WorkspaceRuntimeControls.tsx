...
function buildJobItem(
  ...
  return {
    ...
    title: command.name,
    kind: "job",
    statusLabel: t ? t("workspaceRuntimeControls.runOnce") : "run once",
    lifecycle: null,
    healthStatus: null,
    ...
    serviceIndex: null,
    disabledReason: command.disabledReason ?? (!command.command ? (t ? t("workspaceRuntimeControls.missingCommand") : "This job is missing a command.") : null),
  };
}
...
export function buildWorkspaceRuntimeControlSections(input: {
  ...
  for (const command of commands) {
    if (command.kind === "job") {
      jobs.push(buildJobItem(command, input.canRunJobs ?? input.canStartServices, input.t));
      continue;
    }
  ...
  const otherServices = runtimeServices
    ...
    .map((runtimeService) => ({
      ...
      disabledReason: input.t ? input.t("workspaceRuntimeControls.untrackedService") : "This runtime service no longer matches a configured workspace command.",
    }));
    ...
    <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:flex-wrap">
      {actions.map((action) => {
        ...
        const label = action === "run"
          ? t("workspaceRuntimeControls.run")
          : action === "start"
            ? t("workspaceRuntimeControls.start")
            : action === "stop"
              ? t("workspaceRuntimeControls.stop")
              : t("workspaceRuntimeControls.restart");
            ...
            <div key={item.key} className={cn("border border-border/80 bg-background px-3 py-3", square ? "rounded-none" : "rounded-xl")}>
              <div className="flex flex-col gap-3">
                ...
                <div className="space-y-1 text-xs text-muted-foreground">
                  ...
                  {item.port ? <div>{t("workspaceRuntimeControls.port", { port: item.port })}</div> : null}
...
export function WorkspaceRuntimeControls({
  ...
  const resolvedServiceEmptyMessage = emptyMessage ?? serviceEmptyMessage ?? t("workspaceRuntimeControls.noServicesConfigured");
  const resolvedJobEmptyMessage = jobEmptyMessage ?? t("workspaceRuntimeControls.noJobsConfigured");
  ...
  return (
    <div className={cn("space-y-4", className)}>
      <div className={cn("border border-border/70 bg-background p-3", square ? "rounded-none" : "rounded-xl")}>
        <div className="space-y-1">
          <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{tx("workspaceRuntimeControls.workspaceCommands")}</div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              ...
              {runningCount > 0 ? tx("workspaceRuntimeControls.servicesRunning", { count: runningCount }) : tx("workspaceRuntimeControls.noServicesRunning")}
            </span>
      ...
      <CommandSection
        title={t("workspaceRuntimeControls.services")}
        description={t("workspaceRuntimeControls.servicesDescription")}
      ...
      <CommandSection
        title={t("workspaceRuntimeControls.jobs")}
        description={t("workspaceRuntimeControls.jobsDescription")}
      ...
      {resolvedSections.otherServices.length > 0 ? (
        <CommandSection
          title={t("workspaceRuntimeControls.untrackedServices")}
          description={t("workspaceRuntimeControls.untrackedServicesDescription")}
