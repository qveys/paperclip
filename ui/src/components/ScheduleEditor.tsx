...
const DAY_TRANSLATION_KEY: Record<string, string> = {
  "1": "monday",
  "2": "tuesday",
  "3": "wednesday",
  "4": "thursday",
...
export function ScheduleEditor({
  ...
  const presetLabel = (value: SchedulePreset, fallback: string) =>
    t(`scheduleEditor.preset.${value}`, { defaultValue: fallback });
  const dayLabel = (value: string, fallback: string) =>
    t(`scheduleEditor.day.${DAY_TRANSLATION_KEY[value] ?? value}`, { defaultValue: fallback });
    ...
    <div className="space-y-3">
      <Select value={preset} onValueChange={(v) => handlePresetChange(v as SchedulePreset)}>
        <SelectTrigger className="w-full">
          <SelectValue placeholder={String(t("scheduleEditor.chooseFrequency", { defaultValue: "Choose frequency..." }))} />
        </SelectTrigger>
        <SelectContent>
          {PRESETS.map((p) => (
      ...
      {preset === "custom" ? (
        <div className="space-y-1.5">
          <Input
            ...
            placeholder={String(t("scheduleEditor.customPlaceholder", { defaultValue: "0 10 * * *" }))}
            className="font-mono text-sm"
          />
          <p className="text-xs text-muted-foreground">
            {t("scheduleEditor.customHelp", {
