    ...
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        <h1 className="text-xl font-semibold">
          {t("cloudAccessGate.bootstrap.title", { defaultValue: "Instance setup required" })}
        </h1>
        <p className="mt-2 text-sm text-muted-foreground">
          {hasActiveInvite
            ? t("cloudAccessGate.bootstrap.activeInviteMessage", {
                defaultValue:
                  "No instance admin exists yet. A bootstrap invite is already active. Check your Paperclip startup logs for the first admin invite URL, or run this command to rotate it:",
              })
            : t("cloudAccessGate.bootstrap.noInviteMessage", {
                defaultValue:
...
function NoBoardAccessPage() {
  ...
  return (
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        <h1 className="text-xl font-semibold">
          {t("cloudAccessGate.noAccess.title", { defaultValue: "No company access" })}
        </h1>
        <p className="mt-2 text-sm text-muted-foreground">
          {t("cloudAccessGate.noAccess.membershipMessage", {
            defaultValue:
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          {t("cloudAccessGate.noAccess.inviteMessage", {
      ...
      <div className="mx-auto max-w-xl py-10 text-sm text-destructive">
        {healthQuery.error instanceof Error
          ...
          : boardAccessQuery.error instanceof Error
            ...
            : t("cloudAccessGate.failedState", { defaultValue: "Failed to load app state" })}
      </div>
    );
  }
