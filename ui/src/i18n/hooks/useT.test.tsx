...
// fr-FR is bundled in a later PR (see docs/i18n-rollout.md PR 29). To exercise
// the hook's translated/fallback-en states here, the test ships a minimal
// inline fr-FR catalog containing exactly one translated key. Any other EN
// key resolves through fallback, which is what we want for `fallback-en`.
const FR_INLINE: ResourceLanguage = {
  core: {
    executionWorkspaceDetail: {
      advancedRuntimeJson: TRANSLATED_FR,
    },
  },
};
...
beforeAll(async () => {
  ...
  const enCore = bundledResources.en.core as Record<string, unknown>;
  // Pick an EN-only top-level key that is NOT present in the inline fr-FR
  // catalog (which only contains executionWorkspaceDetail).
  const fallbackCandidate = Object.keys(enCore).find(
    (k) => k !== "executionWorkspaceDetail" && typeof enCore[k] === "string"
  );
  if (!fallbackCandidate) {
    throw new Error("Expected at least one EN-only string key in core namespace for fallback test.");
  }
  EN_ONLY_KEY = fallbackCandidate;
  EN_ONLY_VALUE = enCore[fallbackCandidate] as string;
}, 30000);
