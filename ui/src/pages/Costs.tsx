  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        <CardTitle className="text-base">{t("costs.financeLedger.title", { defaultValue: "Finance ledger" })}</CardTitle>
        <CardDescription>{t("costs.financeLedger.description", { defaultValue: "Account-level charges that do not map to a single inference request." })}</CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        ...
        />
        <MetricTile
          label={t("costs.financeLedger.credits", { defaultValue: "Credits" })}
          value={formatCents(creditCents)}
          subtitle={t("costs.financeLedger.creditsSubtitle", { defaultValue: "Refunds, offsets, and credit returns" })}
          icon={ArrowDownLeft}
        />
        <MetricTile
          label={t("costs.financeLedger.net", { defaultValue: "Net" })}
          value={formatCents(netCents)}
          subtitle={t("costs.financeLedger.netSubtitle", { defaultValue: "Debit minus credit for the selected period" })}
          icon={ReceiptText}
        />
        <MetricTile
          label={t("costs.financeLedger.estimated", { defaultValue: "Estimated" })}
          value={formatCents(estimatedDebitCents)}
          subtitle={t("costs.financeLedger.estimatedSubtitle", { defaultValue: "Estimated debits that are not yet invoice-authoritative" })}
          icon={Coins}
        />
      </CardContent>
    </Card>
...
export function Costs() {
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{tx("costs.allProviders")}</span>
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{tx("costs.allBillers", { defaultValue: "All billers" })}</span>
            {billerKeys.length > 0 ? (
              <>
                <span className="font-mono text-xs text-muted-foreground">{formatTokens(allTokens)}</span>
                <span className="text-xs text-muted-foreground">{formatCents(allCents)}</span>
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={DollarSign} message={t("costs.empty.selectCompany", { defaultValue: "Select a company to view costs." })} />;
  }
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <h1 className="text-3xl font-semibold tracking-tight">{tx("costs.title", { defaultValue: "Costs" })}</h1>
          ...
          {preset === "custom" ? (
            <div className="flex flex-wrap items-center gap-2 border border-border p-3">
              ...
              />
              <span className="text-sm text-muted-foreground">{tx("costs.range.to", { defaultValue: "to" })}</span>
              <input
                type="date"
                value={customTo}
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        <TabsList variant="line" className="justify-start">
          <TabsTrigger value="overview">{tx("costs.tabs.overview", { defaultValue: "Overview" })}</TabsTrigger>
          <TabsTrigger value="budgets">{tx("costs.tabs.budgets", { defaultValue: "Budgets" })}</TabsTrigger>
          <TabsTrigger value="providers">{tx("costs.tabs.providers", { defaultValue: "Providers" })}</TabsTrigger>
          <TabsTrigger value="billers">{tx("costs.tabs.billers", { defaultValue: "Billers" })}</TabsTrigger>
          <TabsTrigger value="finance">{tx("costs.tabs.finance", { defaultValue: "Finance" })}</TabsTrigger>
        </TabsList>

        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{tx("costs.custom.selectDateRange", { defaultValue: "Select a start and end date to load data." })}</p>
          ) : showOverviewLoading ? (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.3fr,1fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{tx("costs.inferenceLedger.title", { defaultValue: "Inference ledger" })}</CardTitle>
                    <CardDescription>
                      {tx("costs.inferenceLedger.description", {
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      ...
                      <div className="border border-border px-4 py-3 text-right">
                        <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">
                          {tx("costs.inferenceLedger.usage", { defaultValue: "usage" })}
                        </div>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("costs.byAgent.title", { defaultValue: "By agent" })}</CardTitle>
                    <CardDescription>{t("costs.byAgent.description", { defaultValue: "What each agent consumed in the selected period." })}</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-2 px-5 pb-5 pt-2">
                    {(spendData?.byAgent.length ?? 0) === 0 ? (
                      <p className="text-sm text-muted-foreground">{t("costs.byAgent.empty", { defaultValue: "No cost events yet." })}</p>
                    ) : (
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("costs.byProject.title", { defaultValue: "By project" })}</CardTitle>
                    <CardDescription>{t("costs.byProject.description", { defaultValue: "Run costs attributed through project-linked issues." })}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                      <p className="text-sm text-muted-foreground">{t("costs.byProject.empty", { defaultValue: "No project-attributed run costs yet." })}</p>
                      ) : (
                        spendData?.byProject.map((row, index) => (
                          ...
                          >
                            <span className="truncate">{row.projectName ?? row.projectId ?? t("costs.byProject.unattributed", { defaultValue: "Unattributed" })}</span>
                  ...
                  <FinanceTimelineCard
                    ...
                    emptyMessage={t("costs.financeTimeline.empty", {
                      defaultValue: "No finance events yet. Add account-level charges once biller invoices or credits land.",
                    })}
                  />
                </div>
              </div>
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">{t("costs.budgets.title", { defaultValue: "Budget control plane" })}</CardTitle>
                  <CardDescription>
                    {t("costs.budgets.description", {
                      defaultValue: "Hard-stop spend limits for agents and projects. Provider subscription quota stays separate and appears under Providers.",
                    })}
                  </CardDescription>
                </CardHeader>
                <CardContent className="grid gap-3 px-5 pb-5 pt-0 md:grid-cols-4">
                  <MetricTile
                    ...
                    subtitle={t("costs.budgets.activeIncidentsSubtitle", { defaultValue: "Open soft or hard threshold crossings" })}
                    icon={ReceiptText}
                  />
                  <MetricTile
                    label={t("costs.budgets.pendingApprovals", { defaultValue: "Pending approvals" })}
                    value={String(budgetData?.pendingApprovalCount ?? 0)}
                    subtitle={t("costs.budgets.pendingApprovalsSubtitle", { defaultValue: "Budget override approvals awaiting board action" })}
                    icon={ArrowUpRight}
                  />
                  <MetricTile
                    label={t("costs.budgets.pausedAgents", { defaultValue: "Paused agents" })}
                    value={String(budgetData?.pausedAgentCount ?? 0)}
                    subtitle={t("costs.budgets.pausedAgentsSubtitle", { defaultValue: "Agent heartbeats blocked by budget" })}
                    icon={Coins}
                  />
                  <MetricTile
                    label={t("costs.budgets.pausedProjects", { defaultValue: "Paused projects" })}
                    value={String(budgetData?.pausedProjectCount ?? 0)}
                    subtitle={t("costs.budgets.pausedProjectsSubtitle", { defaultValue: "Project execution blocked by budget" })}
                    icon={DollarSign}
                  />
                </CardContent>
              </Card>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    <h2 className="text-lg font-semibold">{t("costs.budgets.activeIncidentsHeading", { defaultValue: "Active incidents" })}</h2>
                    <p className="text-sm text-muted-foreground">
                      {t("costs.budgets.activeIncidentsHelp", {
                ...
                {budgetPolicies.length === 0 ? (
                  <Card>
                    <CardContent className="px-5 py-8 text-sm text-muted-foreground">
                      {t("costs.budgets.empty", {
                        defaultValue: "No budget policies yet. Set agent and project budgets from their detail pages, or use the existing company monthly budget control.",
                      })}
                    </CardContent>
                  </Card>
                ) : null}
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("costs.providers.empty", { defaultValue: "No cost events in this period." })}</p>
                  ) : (
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("costs.billers.empty", { defaultValue: "No billable events in this period." })}</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.2fr,0.95fr]">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("costs.financeByBiller.title", { defaultValue: "By biller" })}</CardTitle>
                      <CardDescription>{t("costs.financeByBiller.description", { defaultValue: "Account-level financial events grouped by who charged or credited them." })}</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 px-5 pb-5 pt-2 md:grid-cols-2">
                      {(financeData?.byBiller.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("costs.financeByBiller.empty", { defaultValue: "No finance events yet." })}</p>
                      ) : (
