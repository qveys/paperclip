...
export function RoutineDetail() {
  ...
  useEffect(() => {
    if (!routine) return;
    setBreadcrumbs([{ label: t("routines.title", { defaultValue: "Routines" }), href: "/routines" }, { label: routine.title }]);
  ...
  const updateRoutineStatus = useMutation({
    ...
    onSuccess: async (_data, status) => {
      pushToast({
        title: t("routineDetail.toast.saved", { defaultValue: "Routine saved" }),
        body: status === "paused"
  ...
  const createTrigger = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: t("routineDetail.toast.triggerAddFailed", { defaultValue: "Failed to add trigger" }),
        body: error instanceof Error ? error.message : t("routineDetail.toast.triggerAddFailedBody", { defaultValue: "Paperclip could not create the trigger." }),
        tone: "error",
      });
    },
  });
  ...
  const updateTrigger = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: t("routineDetail.toast.triggerUpdateFailed", { defaultValue: "Failed to update trigger" }),
        body: error instanceof Error ? error.message : t("routineDetail.toast.triggerUpdateFailedBody", { defaultValue: "Paperclip could not update the trigger." }),
        tone: "error",
      });
    },
  });
  ...
  const deleteTrigger = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: t("routineDetail.toast.triggerDeleteFailed", { defaultValue: "Failed to delete trigger" }),
        body: error instanceof Error ? error.message : t("routineDetail.toast.triggerDeleteFailedBody", { defaultValue: "Paperclip could not delete the trigger." }),
        tone: "error",
      });
    },
  });
  ...
  const rotateTrigger = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: t("routineDetail.toast.rotateSecretFailed", { defaultValue: "Failed to rotate webhook secret" }),
        body: error instanceof Error ? error.message : t("routineDetail.toast.rotateSecretFailedBody", { defaultValue: "Paperclip could not rotate the webhook secret." }),
        tone: "error",
      });
    },
  });
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Repeat} message={t("routines.empty.selectCompany", { defaultValue: "Select a company to view routines." })} />;
  }
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div className="flex items-start gap-4">
        <textarea
          ...
          placeholder={t("routineDetail.placeholders.title", { defaultValue: "Routine title" })}
          rows={1}
        ...
        <div className="flex shrink-0 items-center gap-3 pt-1">
          ...
          <ToggleSwitch
            ...
            aria-label={automationEnabled
              ? t("routineDetail.actions.pauseAutomaticTriggers", { defaultValue: "Pause automatic triggers" })
              : t("routineDetail.actions.enableAutomaticTriggers", { defaultValue: "Enable automatic triggers" })}
          />
        ...
        <div className="rounded-lg border border-blue-500/30 bg-blue-500/5 p-4 space-y-3 text-sm">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              {t("routineDetail.secret.saveNow", { defaultValue: "Save this now. Paperclip will not show the secret value again." })}
            </p>
          </div>
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <Input value={secretMessage.webhookUrl} readOnly className="flex-1" />
              <Button variant="outline" size="sm" onClick={() => copySecretValue(t("routineDetail.secret.webhookUrl", { defaultValue: "Webhook URL" }), secretMessage.webhookUrl)}>
                <Copy className="h-3.5 w-3.5 mr-1" />
                {t("routineDetail.secret.url", { defaultValue: "URL" })}
              </Button>
            </div>
            <div className="flex items-center gap-2">
              <Input value={secretMessage.webhookSecret} readOnly className="flex-1" />
              <Button variant="outline" size="sm" onClick={() => copySecretValue(t("routineDetail.secret.webhookSecret", { defaultValue: "Webhook secret" }), secretMessage.webhookSecret)}>
                <Copy className="h-3.5 w-3.5 mr-1" />
                {t("routineDetail.secret.secret", { defaultValue: "Secret" })}
              </Button>
            </div>
          </div>
        </div>
      ...
      <MarkdownEditor
        ...
        placeholder={t("routineDetail.placeholders.instructions", { defaultValue: "Add instructions..." })}
        bordered={false}
      ...
      <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
        <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
          <span className="text-sm font-medium">{t("routineDetail.advanced.title", { defaultValue: "Advanced delivery settings" })}</span>
          {advancedOpen ? <ChevronDown className="h-4 w-4 text-muted-foreground" /> : <ChevronRight className="h-4 w-4 text-muted-foreground" />}
        </CollapsibleTrigger>
        <CollapsibleContent className="pt-3">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("routineDetail.advanced.concurrency", { defaultValue: "Concurrency" })}</p>
              <Select
      ...
      <div className="flex items-center justify-between">
        ...
        >
          ...
          {t("routineDetail.actions.saveRoutine", { defaultValue: "Save routine" })}
        </Button>
      </div>
...
<TabsTrigger value="activity" className="gap-1.5">
        ...
        <TabsContent value="triggers" className="space-y-4">
          ...
          <div className="rounded-lg border border-border p-4 space-y-3">
            <p className="text-sm font-medium">{t("routineDetail.triggers.addTrigger", { defaultValue: "Add trigger" })}</p>
            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-1.5">
                <Label className="text-xs">{t("routineDetail.triggers.kind", { defaultValue: "Kind" })}</Label>
              ...
              {newTrigger.kind === "schedule" && (
                <div className="md:col-span-2 space-y-1.5">
                  <Label className="text-xs">{t("routineDetail.triggers.schedule", { defaultValue: "Schedule" })}</Label>
                  <ScheduleEditor
              ...
              {newTrigger.kind === "webhook" && (
                <>
                  <div className="space-y-1.5">
                    <Label className="text-xs">{t("routineDetail.triggers.signingMode", { defaultValue: "Signing mode" })}</Label>
                  ...
                  {!SIGNING_MODES_WITHOUT_REPLAY_WINDOW.has(newTrigger.signingMode) && (
                    <div className="space-y-1.5">
                      <Label className="text-xs">{t("routineDetail.triggers.replayWindow", { defaultValue: "Replay window (seconds)" })}</Label>
            ...
            <div className="flex items-center justify-end">
              <Button size="sm" onClick={() => createTrigger.mutate()} disabled={createTrigger.isPending}>
                {createTrigger.isPending
                  ? t("routineDetail.triggers.adding", { defaultValue: "Adding..." })
                  : t("routineDetail.triggers.addTrigger", { defaultValue: "Add trigger" })}
              </Button>
            </div>
          </div>
          ...
          {routine.triggers.length === 0 ? (
            <p className="text-xs text-muted-foreground">{t("routineDetail.triggers.empty", { defaultValue: "No triggers configured yet." })}</p>
          ) : (
        ...
        <TabsContent value="runs" className="space-y-4">
          ...
          {(routineRuns ?? []).length === 0 ? (
            <p className="text-xs text-muted-foreground">{t("routineDetail.runs.empty", { defaultValue: "No runs yet." })}</p>
          ) : (
        ...
        <TabsContent value="activity">
          {(activity ?? []).length === 0 ? (
            <p className="text-xs text-muted-foreground">{t("routineDetail.activity.empty", { defaultValue: "No activity yet." })}</p>
          ) : (
            <div className="border border-border rounded-lg divide-y divide-border">
              {(activity ?? []).map((event) => (
