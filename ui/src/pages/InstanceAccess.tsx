...
export function InstanceAccess() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("instanceAccess.breadcrumb.instanceSettings", { defaultValue: "Instance Settings" }), href: "/instance/settings/general" },
      { label: t("instanceAccess.breadcrumb.access", { defaultValue: "Access" }) },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const setAdminMutation = useMutation({
    mutationFn: async (makeAdmin: boolean) => {
      if (!selectedUserId) throw new Error(t("instanceAccess.noUserSelected", { defaultValue: "No user selected" }));
    ...
    const message =
      usersQuery.error instanceof ApiError && usersQuery.error.status === 403
        ...
        : usersQuery.error instanceof Error
          ...
          : t("instanceAccess.loadUsersFailed", { defaultValue: "Failed to load users." });
    ...
    <div className="max-w-6xl space-y-6">
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("instanceAccess.title", { defaultValue: "Instance Access" })}</h1>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[320px_minmax(0,1fr)]">
            ...
            {(usersQuery.data ?? []).map((user) => (
              ...
              >
                ...
                <div className="mt-2 text-xs text-muted-foreground">
                  {t("instanceAccess.activeCompanyMemberships", {
        ...
        <section className="space-y-4 rounded-xl border border-border bg-card p-5">
          {!selectedUserId ? (
            <div className="text-sm text-muted-foreground">{t("instanceAccess.selectUserHint", { defaultValue: "Select a user to inspect instance access." })}</div>
            ...
            <div className="text-sm text-destructive">
              {userAccessQuery.error instanceof Error
                ...
                : t("instanceAccess.loadUserAccessFailed", { defaultValue: "Failed to load user access." })}
            </div>
          ) : (
            <>
              <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                >
                  {selectedUser?.isInstanceAdmin
                    ...
                    : t("instanceAccess.promoteInstanceAdmin", { defaultValue: "Promote to instance admin" })}
                </Button>
              </div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {t("instanceAccess.companyAccessDescription", {
                    ...
                    >
                      <div>
                        ...
                        <div className="text-muted-foreground">
                          {(membership.membershipRole || t("instanceAccess.unset", { defaultValue: "unset" }))} • {membership.status}
                        </div>
                      </div>
