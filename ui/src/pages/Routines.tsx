...
export function Routines() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("routines.title", { defaultValue: "Routines" }) }]);
  }, [setBreadcrumbs, t]);
  ...
  const createRoutine = useMutation({
    ...
    onSuccess: async (routine) => {
      ...
      pushToast({
        title: t("routines.toast.created", { defaultValue: "Routine created" }),
        body: routine.assigneeAgentId
          ? t("routines.toast.createdWithAssignee", { defaultValue: "Add the first trigger to turn it into a live workflow." })
          : t("routines.toast.createdDraft", { defaultValue: "Draft saved. Add a default agent before enabling automation." }),
        tone: "success",
      });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routines.toast.updateFailed", { defaultValue: "Failed to update routine" }),
        body: mutationError instanceof Error ? mutationError.message : t("routines.toast.updateFailedBody", { defaultValue: "Paperclip could not update the routine." }),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routines.toast.runFailed", { defaultValue: "Routine run failed" }),
        body: mutationError instanceof Error ? mutationError.message : t("routines.toast.runFailedBody", { defaultValue: "Paperclip could not start the routine run." }),
        tone: "error",
      });
    },
  });
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div className="space-y-1">
          <h1 className="text-2xl font-semibold tracking-tight">
            {t("routines.title", { defaultValue: "Routines" })}
          </h1>
          <p className="text-sm text-muted-foreground">
            {t("routines.subtitle", { defaultValue: "Recurring work definitions that materialize into auditable execution issues." })}
          </p>
        </div>
        <Button onClick={() => setComposerOpen(true)}>
          <Plus className="mr-2 h-4 w-4" />
          {t("routines.actions.create", { defaultValue: "Create routine" })}
        </Button>
      </div>
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            <div className="px-5 pt-5 pb-3">
              <textarea
                ...
                placeholder={t("routines.placeholders.title", { defaultValue: "Routine title" })}
                rows={1}
                value={draft.title}
                onChange={(event) => {
              ...
              <div className="overflow-x-auto overscroll-x-contain">
                <div className="inline-flex min-w-full flex-wrap items-center gap-2 text-sm text-muted-foreground sm:min-w-max sm:flex-nowrap">
                  ...
                  <InlineEntitySelector
                    ...
                    placeholder={t("routines.assignee", { defaultValue: "Assignee" })}
                    noneLabel={t("routines.noAssignee", { defaultValue: "No assignee" })}
                    searchPlaceholder={t("routines.searchAssignees", { defaultValue: "Search assignees..." })}
                    emptyMessage={t("routines.noAssigneesFound", { defaultValue: "No assignees found." })}
            ...
            <div className="border-t border-border/60 px-5 py-4">
              <MarkdownEditor
                ...
                placeholder={t("routines.placeholders.instructions", { defaultValue: "Add instructions..." })}
                bordered={false}
            ...
            <div className="border-t border-border/60 px-5 py-3">
              <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
                <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
                  <div>
                    <p className="text-sm font-medium">{t("routines.advanced.title", { defaultValue: "Advanced delivery settings" })}</p>
                    <p className="text-sm text-muted-foreground">{t("routines.advanced.hint", { defaultValue: "Keep policy controls secondary to the work definition." })}</p>
                  </div>
                ...
                <CollapsibleContent className="pt-3">
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2">
                      <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("routines.advanced.concurrency", { defaultValue: "Concurrency" })}</p>
                      <Select
            ...
            <div className="flex flex-col gap-2 sm:items-end">
              ...
              {createRoutine.isError ? (
                <p className="text-sm text-destructive">
                  {createRoutine.error instanceof Error ? createRoutine.error.message : t("routines.errors.create", { defaultValue: "Failed to create routine" })}
                </p>
              ) : null}
            </div>
          </div>
