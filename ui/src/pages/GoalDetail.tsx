...
export function GoalDetail() {
  ...
  const uploadImage = useMutation({
    mutationFn: async (file: File) => {
      if (!resolvedCompanyId) throw new Error(t("goals.detail.errors.noCompanySelected"));
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          <span className="text-xs uppercase text-muted-foreground">
            {tx(`goals.properties.levelValues.${goal.level}`)}
          </span>
      ...
      <Tabs defaultValue="children">
        <TabsList>
          <TabsTrigger value="children">
            {tx("goals.detail.subGoalsCount", { count: childGoals.length })}
          </TabsTrigger>
          <TabsTrigger value="projects">
            {tx("goals.detail.projectsCount", { count: linkedProjects.length })}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="children" className="mt-4 space-y-3">
          <div className="flex items-center justify-start">
            ...
            >
              ...
              {tx("goals.detail.subGoalButton")}
            </Button>
          </div>
          {childGoals.length === 0 ? (
            <p className="text-sm text-muted-foreground">{tx("goals.detail.noSubGoals")}</p>
          ) : (
        ...
        <TabsContent value="projects" className="mt-4">
          {linkedProjects.length === 0 ? (
            <p className="text-sm text-muted-foreground">{tx("goals.detail.noLinkedProjects")}</p>
          ) : (
