...
export function InstanceSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("instanceSettings.breadcrumb.instanceSettings", { defaultValue: "Instance Settings" }) },
      { label: t("instanceSettings.breadcrumb.heartbeats", { defaultValue: "Heartbeats" }) },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.errors.updateHeartbeat", { defaultValue: "Failed to update heartbeat." }));
    },
  });
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        const detail = firstError instanceof Error ? firstError.message : t("instanceSettings.errors.unknownError", { defaultValue: "Unknown error" });
        throw new Error(
          failures.length === 1
            ? t("instanceSettings.errors.disableOneHeartbeat", {
                defaultValue: "Failed to disable 1 timer heartbeat: {{detail}}",
                detail,
              })
            : t("instanceSettings.errors.disableSomeHeartbeats", {
                defaultValue: "Failed to disable {{failures}} of {{enabled}} timer heartbeats. First error: {{detail}}",
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.errors.disableAllHeartbeats", { defaultValue: "Failed to disable all heartbeats." }));
    },
  });
  ...
  if (heartbeatsQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("instanceSettings.loading", { defaultValue: "Loading scheduler heartbeats..." })}</div>;
  }
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : t("instanceSettings.errors.loadHeartbeats", { defaultValue: "Failed to load scheduler heartbeats." })}
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("instanceSettings.title", { defaultValue: "Scheduler Heartbeats" })}</h1>
        </div>
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span><span className="font-semibold text-foreground">{activeCount}</span> {t("instanceSettings.active", { defaultValue: "active" })}</span>
        <span><span className="font-semibold text-foreground">{disabledCount}</span> {t("instanceSettings.disabled", { defaultValue: "disabled" })}</span>
        <span>
          <span className="font-semibold text-foreground">{grouped.length}</span>{" "}
          {grouped.length === 1
            ? t("instanceSettings.companySingular", { defaultValue: "company" })
            : t("instanceSettings.companyPlural", { defaultValue: "companies" })}
        </span>
        {anyEnabled && (
          <Button
            variant="destructive"
            ...
            onClick={() => {
              const noun = enabledCount === 1
                ? t("instanceSettings.agentSingular", { defaultValue: "agent" })
                : t("instanceSettings.agentPlural", { defaultValue: "agents" });
              if (!window.confirm(t("instanceSettings.confirmDisableAll", {
                defaultValue: "Disable timer heartbeats for all {{count}} enabled {{noun}}?",
                count: enabledCount,
                noun,
              }))) {
                return;
          ...
          >
            {disableAllMutation.isPending
              ? t("instanceSettings.disabling", { defaultValue: "Disabling..." })
              : t("instanceSettings.disableAll", { defaultValue: "Disable All" })}
          </Button>
        )}
      </div>
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={t("instanceSettings.empty", { defaultValue: "No scheduler heartbeats match the current criteria." })}
        />
      ) : (
                      ...
                      >
                        ...
                        >
                          {agent.schedulerActive
                            ? t("instanceSettings.on", { defaultValue: "On" })
                            : t("instanceSettings.off", { defaultValue: "Off" })}
                        </Badge>
                        <Link
                        ...
                        >
                          {agent.lastHeartbeatAt
                            ...
                            : t("instanceSettings.never", { defaultValue: "never" })}
                        </span>
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          <Link
                            to={buildAgentHref(agent)}
                            className="text-muted-foreground hover:text-foreground"
                            title={t("instanceSettings.fullAgentConfig", { defaultValue: "Full agent config" })}
                          >
                          ...
                          >
                            {saving
                              ...
                              : agent.heartbeatEnabled
                                ? t("instanceSettings.disableTimerHeartbeat", { defaultValue: "Disable Timer Heartbeat" })
                                : t("instanceSettings.enableTimerHeartbeat", { defaultValue: "Enable Timer Heartbeat" })}
                          </Button>
                        </span>
                      </div>
                    );
