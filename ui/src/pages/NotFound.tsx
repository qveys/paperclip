...
export function NotFoundPage({ scope = "global", requestedPrefix }: NotFoundPageProps) {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("notFound.breadcrumb", { defaultValue: "Not Found" }) }]);
  }, [setBreadcrumbs, t]);
  ...
  const title =
    scope === "invalid_company_prefix"
      ? t("notFound.companyNotFound", { defaultValue: "Company not found" })
      : t("notFound.pageNotFound", { defaultValue: "Page not found" });
  const description =
    scope === "invalid_company_prefix"
      ? t("notFound.noCompanyPrefixMatch", {
          defaultValue: `No company matches prefix "${normalizedPrefix ?? "unknown"}".`,
          prefix: normalizedPrefix ?? "unknown",
        })
      : t("notFound.routeMissing", { defaultValue: "This route does not exist." });
    ...
    <div className="mx-auto max-w-2xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        ...
        <div className="mt-4 rounded-md border border-border bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
          {t("notFound.requestedPath", { defaultValue: "Requested path:" })}{" "}
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          <Button asChild>
            <Link to={dashboardHref}>
              ...
              {t("notFound.openDashboard", { defaultValue: "Open dashboard" })}
            </Link>
          </Button>
          <Button variant="outline" asChild>
            <Link to="/">{t("notFound.goHome", { defaultValue: "Go home" })}</Link>
          </Button>
        </div>
      </div>
    </div>
