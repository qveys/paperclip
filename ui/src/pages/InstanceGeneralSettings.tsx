...
export function InstanceGeneralSettings() {
  ...
  const signOutMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceGeneral.errors.signOutFailed", { defaultValue: "Failed to sign out." }));
    },
  });

  useEffect(() => {
    setBreadcrumbs([
      { label: t("instanceGeneral.breadcrumb.instanceSettings", { defaultValue: "Instance Settings" }) },
      { label: t("instanceGeneral.breadcrumb.general", { defaultValue: "General" }) },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const updateGeneralMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceGeneral.errors.updateFailed", { defaultValue: "Failed to update general settings." }));
    },
  });

  if (generalQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("instanceGeneral.loading", { defaultValue: "Loading general settings..." })}</div>;
  }
    ...
    return (
      <div className="text-sm text-destructive">
        {generalQuery.error instanceof Error
          ...
          : t("instanceGeneral.errors.loadFailed", { defaultValue: "Failed to load general settings." })}
      </div>
    );
  }
    ...
    <div className="max-w-4xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("instanceGeneral.title", { defaultValue: "General" })}</h1>
        </div>
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            <h2 className="text-sm font-semibold">{t("instanceGeneral.deploymentAndAuth", { defaultValue: "Deployment and auth" })}</h2>
            <ModeBadge
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="flex items-start justify-between gap-4">
          ...
          <ToggleSwitch
            ...
            aria-label={t("instanceGeneral.toggleUsernameLogCensoring", { defaultValue: "Toggle username log censoring" })}
          />
        </div>
      </section>
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="flex items-start justify-between gap-4">
          ...
          <ToggleSwitch
            ...
            disabled={updateGeneralMutation.isPending}
            aria-label={t("instanceGeneral.toggleKeyboardShortcuts", { defaultValue: "Toggle keyboard shortcuts" })}
          />
        </div>
      </section>
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="space-y-5">
          ...
          <div className="space-y-1.5">
            <h3 className="text-xs font-medium text-muted-foreground uppercase tracking-wide">{t("instanceGeneral.daily", { defaultValue: "Daily" })}</h3>
            <div className="flex flex-wrap gap-2">
              {DAILY_RETENTION_PRESETS.map((days) => {
                ...
                return (
                  ...
                  >
                    <div className="text-sm font-medium">{t("instanceGeneral.daysLabel", { defaultValue: "{{days}} days", days })}</div>
                  </button>
                );
              })}
            </div>
          ...
          <div className="space-y-1.5">
            <h3 className="text-xs font-medium text-muted-foreground uppercase tracking-wide">{t("instanceGeneral.weekly", { defaultValue: "Weekly" })}</h3>
            <div className="flex flex-wrap gap-2">
              {WEEKLY_RETENTION_PRESETS.map((weeks) => {
                const active = backupRetention.weeklyWeeks === weeks;
                const label = weeks === 1
                  ? t("instanceGeneral.oneWeek", { defaultValue: "1 week" })
                  : t("instanceGeneral.weeksLabel", { defaultValue: "{{weeks}} weeks", weeks });
                return (
                  <button
                    key={weeks}
                    type="button"
          ...
          <div className="space-y-1.5">
            <h3 className="text-xs font-medium text-muted-foreground uppercase tracking-wide">{t("instanceGeneral.monthly", { defaultValue: "Monthly" })}</h3>
            <div className="flex flex-wrap gap-2">
              {MONTHLY_RETENTION_PRESETS.map((months) => {
                const active = backupRetention.monthlyMonths === months;
                const label = months === 1
                  ? t("instanceGeneral.oneMonth", { defaultValue: "1 month" })
                  : t("instanceGeneral.monthsLabel", { defaultValue: "{{months}} months", months });
                return (
                  <button
                    key={months}
                    type="button"
        ...
        <div className="space-y-4">
          ...
          {feedbackDataSharingPreference === "prompt" ? (
            <div className="rounded-lg border border-border/70 bg-accent/20 px-3 py-2 text-sm text-muted-foreground">
              {t("instanceGeneral.noDefaultFeedbackPreference", {
            ...
            {[
              {
                ...
                description: t("instanceGeneral.alwaysAllowDescription", { defaultValue: "Share voted AI outputs automatically." }),
              },
              {
                value: "not_allowed",
                label: t("instanceGeneral.doNotAllow", { defaultValue: "Don't allow" }),
                description: t("instanceGeneral.doNotAllowDescription", { defaultValue: "Keep voted AI outputs local only." }),
              },
            ].map((option) => {
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          >
            ...
            {signOutMutation.isPending
              ? t("instanceGeneral.signingOut", { defaultValue: "Signing out..." })
              : t("instanceGeneral.signOut", { defaultValue: "Sign out" })}
          </Button>
        </div>
      </section>
    </div>
